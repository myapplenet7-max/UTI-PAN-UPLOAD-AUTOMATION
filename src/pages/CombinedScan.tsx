...
Only return the objects you are highly confident about. Do not include items you cannot clearly see.`;
      ...
      const response = await callAI(apiKeys, prompt, selectedAi, autoFailover, b64, file.type, true);
...
Rules: For names, use exact spelling as printed. For dates, use DD-MM-YYYY format. If you cannot read a field, return null.`
              ...
              const response = await callAI(apiKeys, prompt, selectedAi, autoFailover, b64, 'image/jpeg', true)
    ...
    <div className="page">
      ...
      {status === 'extracted' && (
        <div className="card" style={{ borderColor: 'var(--accent)' }}>
          ...
          {!matching && matches.length > 0 && (<><p style={{ fontSize: 12, color: 'var(--text3)', margin: '8px 0' }}>Found {matches.length} possible existing customer record(s) — merge into one, or create new below.</p><div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>{matches.map(m => (<button key={m.id} onClick={() => setSelectedId(m.id)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', borderRadius: 8, textAlign: 'left', border: `1px solid ${selectedId === m.id ? 'var(--accent)' : 'var(--border)'}`, background: selectedId === m.id ? 'rgba(59,110,245,0.18)' : 'var(--bg3)', cursor: 'pointer', color: 'var(--text)', fontFamily: 'inherit' }}><div><div style={{ fontWeight: 600, fontSize: 13, color: 'var(--text)' }}>{m.full_name || '(no name)'}</div><div style={{ fontSize: 11, color: 'var(--text2)', marginTop: 2 }}>{m.aadhaar_number && `Aadhaar: ${m.aadhaar_number}`}{m.pan_number && `  PAN: ${m.pan_number}`} · {m.document_count} on file</div></div>{selectedId === m.id && <Check size={16} color="var(--accent)" />}</button>))}</div>{selectedId && (<button className="btn btn-primary" onClick={() => saveUnderApplicant(selectedId)} style={{ marginBottom: 12 }}><Check size={14} /> Merge into Selected Customer</button>)}</>)}
          ...
          {manualResults.map(m => (<button key={m.id} onClick={() => saveUnderApplicant(m.id)} style={{ display: 'block', width: '100%', textAlign: 'left', padding: '8px 12px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg3)', color: 'var(--text)', fontFamily: 'inherit', marginBottom: 6, cursor: 'pointer' }}>{m.full_name}</button>))}
        </div>
      )}
